...
import com.yf.accountmanager.action.BaseAction;
...
public class CustomizedDialog extends Dialog {
	...
	public static final int resId = R.layout.dialog_customized,
	        ...
	        fileSystemUsageDialogResId = R.layout.dialog_filesystemusage;
	...
	// searchUsageDialog
	public static FileSystemUsageDialog initFileSystemUsageDialog(
			Context context,BaseAdapter adapter) {
		return new FileSystemUsageDialog(context,adapter);
	}
	...
	public static class FileSystemUsageDialog extends Dialog implements
			View.OnClickListener {
		...
		public FileSystemUsageDialog(Context context,BaseAdapter adapter) {
		...
		protected void onCreate(Bundle savedInstanceState) {
			super.onCreate(savedInstanceState);
			View contentView = LayoutInflater.from(getContext()).inflate(
					fileSystemUsageDialogResId, null, false);
			setContentView(
					contentView,
					new LayoutParams(getContext().getResources()
							.getDimensionPixelSize(R.dimen.dp_320),
